...
function ResponsiveSidebar() {
  ...
  // Solo 3 Hubs principales
  const hubItems = [
    { name: "Status del Restaurant", path: '/dashboard/restaurant-status', icon: FaUtensils },
  ...
  return isMobile ? (
      ...
      <HStack justify="space-around">
        {hubItems.map((hub) => (
          <Tooltip key={hub.name} label={hub.name} aria-label={hub.name}>
            <Button as={Link} to={hub.path} variant="link" color="whiteAlpha.900" p={2}>
              <hub.icon size="24px" />
            </Button>
          </Tooltip>
        ))}
    ...
    <Box as="aside" width="250px" bg="#444" p={4}>
      <VStack align="start" spacing={4}>
        {hubItems.map((hub) => (
          <Button key={hub.name} as={Link} to={hub.path} variant="link" color="whiteAlpha.900">
            <HStack spacing={2}>
              <hub.icon />
              <span>{hub.name}</span>
            </HStack>
